...
mongodb.MongoClient.connect(uri, function(err, client) {
  ...
  let sanctions = db.collection('sanctions_test');

   // Note that the insert method can take either an array or a dict.


  sanctions.find( { "nameAlias.lastName" : "KHODAKOVSKY" } ).toArray(function (err, docs) {
     ...
     docs.forEach(function (doc) {
       console.log(
         ...
         ', First name: ' +
         JSON.stringify(doc.nameAlias[0].firstName) +
         ', Last name: ' +
         JSON.stringify(doc.nameAlias[0].lastName) +
        ', Whole name: ' +
         JSON.stringify(doc.nameAlias[0].wholeName)
       );
     });
